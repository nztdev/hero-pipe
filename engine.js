...
(function() {
  ...
  function render(config, canvas) {
    ...
    var ctrl = {
      ...
      getScroll: function() { return autoScrollP; },
  ...
  function buildExportHTML(config) {
    ...
    return [
      ...
      '    .hp-glass-panel{position:relative;padding:2rem 2.5rem;border-radius:20px;background:rgba(255,255,255,0.045);border:1px solid rgba(255,255,255,0.09);backdrop-filter:blur(18px) saturate(160%);-webkit-backdrop-filter:blur(18px) saturate(160%);overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.25),inset 0 1px 0 rgba(255,255,255,0.08);display:flex;flex-direction:column;align-items:center;}',
      '    .hp-glass-panel::before{content:\'\';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:linear-gradient(115deg,transparent 40%,rgba(255,255,255,0.06) 50%,transparent 60%);animation:hpGlassSheen 7s ease-in-out infinite;pointer-events:none;}',
      '    @keyframes hpGlassSheen{0%,100%{transform:translate(-30%,-10%);}50%{transform:translate(30%,10%);}}',
      '    @supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){.hp-glass-panel{background:rgba(10,10,20,0.55);}}',
      '    #hero-headline{',
      ...
      '      transition:opacity 0.6s ease 0.8s,transform 0.75s cubic-bezier(0.34,1.56,0.64,1) 0.8s,background 0.2s;',
      '    }',
      '    #hero-cta.in{opacity:1;transform:translateY(0) scale(1);}',
      ...
      '    /* spring physics: headline letters react to scroll velocity, then settle */',
      '    function heroPipeSpringText(el,text){',
      '      el.textContent="";',
      '      var spans=[];',
      ...
      '        sp.style.display="inline-block";sp.style.willChange="transform";',
      '        el.appendChild(sp);',
      '        spans.push({el:sp,y:0,vy:0,rot:0,vr:0});',
      '      }',
      '      var lastP=null;',
      '      return function(p){',
      '        if(el.children.length!==spans.length)return;',
      '        if(lastP===null)lastP=p;',
      '        var dv=(p-lastP)*900;lastP=p;',
      '        var n=spans.length;',
      '        for(var i=0;i<n;i++){',
      '          var s=spans[i];var w=1-(i/Math.max(1,n-1))*0.5;',
      '          var tY=dv*w;var fY=(tY-s.y)*0.18;s.vy=(s.vy+fY)*0.82;s.y+=s.vy;',
      '          var tR=dv*0.35*w;var fR=(tR-s.rot)*0.18;s.vr=(s.vr+fR)*0.82;s.rot+=s.vr;',
      '          s.el.style.transform="translateY("+s.y.toFixed(2)+"px) rotateZ("+s.rot.toFixed(2)+"deg)";',
      '        }',
      '      };',
      '    }',
      '    var springUpdate=null;',
      '    var headlineEl=document.getElementById("hero-headline");',
      '    if(headlineEl)springUpdate=heroPipeSpringText(headlineEl,config.headline||"");',
      '    (function physicsLoop(){',
      '      requestAnimationFrame(physicsLoop);',
      '      var pp=window.scrollY/(document.body.scrollHeight-window.innerHeight)||0;',
      '      if(springUpdate)springUpdate(pp);',
      '    })();',
